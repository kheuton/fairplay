...
function buildIcons() {
  ...
  const rr  = (x: N, y: N, w: N, h: N, r: N, f: string, o: N = '') => `<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="${r}" fill="${f}"${o?` opacity="${o}"`:''}/>`;
  const pa  = (d: string, f: string, o: N = '')         => `<path d="${d}" fill="${f}"${o?` opacity="${o}"`:''}/>`;
  const ci  = (cx: N, cy: N, r: N, f: string, o: N = '')   => `<circle cx="${cx}" cy="${cy}" r="${r}" fill="${f}"${o?` opacity="${o}"`:''}/>`;
  const sheen = (x: number, y: number, h: number, f: string = C.sheen, w: number = 1.4) => rr((x+0.85).toFixed(2),(y+0.7).toFixed(2),w,(h-1.5).toFixed(2),0.7,f,0.5);
  // teardrop centered at (cx,cy) top, scale s
  const drop = (cx: number, cy: number, s: number, f: string) => pa(`M${cx} ${cy} C${(cx+0.95*s).toFixed(2)} ${(cy+1.15*s).toFixed(2)} ${(cx+1.02*s).toFixed(2)} ${(cy+1.95*s).toFixed(2)} ${(cx+1.02*s).toFixed(2)} ${(cy+2.4*s).toFixed(2)} a${(1.02*s).toFixed(2)} ${(1.02*s).toFixed(2)} 0 0 1 ${(-2.04*s).toFixed(2)} 0 C${(cx-1.02*s).toFixed(2)} ${(cy+1.95*s).toFixed(2)} ${(cx-0.95*s).toFixed(2)} ${(cy+1.15*s).toFixed(2)} ${cx} ${cy} Z`, f);
  const lines = (x: number, y: number, w: number, f: string) => rr(x,y,w,0.85,0.4,f,0.85)+rr(x,(y+1.7).toFixed(2),(w*0.7).toFixed(2),0.85,0.4,f,0.6);
  // labelled body content block
  const star4 = (cx: number, cy: number, s: number, f: string) => pa(`M${cx} ${cy-s} L${(cx+0.34*s).toFixed(2)} ${(cy-0.34*s).toFixed(2)} L${cx+s} ${cy} L${(cx+0.34*s).toFixed(2)} ${(cy+0.34*s).toFixed(2)} L${cx} ${cy+s} L${(cx-0.34*s).toFixed(2)} ${(cy+0.34*s).toFixed(2)} L${cx-s} ${cy} L${(cx-0.34*s).toFixed(2)} ${(cy-0.34*s).toFixed(2)} Z`, f);
  const leaf = (cx: number, cy: number, s: number, f: string) => pa(`M${cx} ${cy} C${cx+2.6*s} ${cy+0.2*s} ${cx+2.6*s} ${cy+3.4*s} ${cx-0.2*s} ${cy+4*s} C${cx-1*s} ${cy+2*s} ${cx-0.9*s} ${cy+1*s} ${cx} ${cy} Z`, f);

  const I: Record<string, { label: string; cat: string; svg: string }> = {};
  ...
  const zipBag = (topY: number, zipColor: string, fillN: number, sandwich: boolean) => {
  ...
  /* ════════════ BATH ════════════ */
  const bottle = (bodyC: string, bodyD: string, capC: string, capD: string, content: string, topY: number = 8.4, capH: number = 2.4) => {
  ...
  const sprayer = (bodyC: string, bodyD: string, content: string) =>
  ...
  const pillBottle = (capC: string, bodyC: string, bodyD: string, content: string) =>
  ...
  return { RICH_ICONS: I, RICH_ORDER: Object.keys(I) };
}

const built = buildIcons();
export const RICH_ICONS = built.RICH_ICONS;
export const RICH_ORDER = built.RICH_ORDER;
